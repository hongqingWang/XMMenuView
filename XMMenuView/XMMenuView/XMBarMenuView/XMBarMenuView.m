...
@interface XMBarMenuView()<UITableViewDelegate, UITableViewDataSource>
{
    CGPoint          _refPoint;
    UIView          *_refView;
    CGFloat          _menuWidth;
...
#pragma mark - XMBarMenuView - implementation
@implementation XMBarMenuView


+ (instancetype)menuWithArray:(NSArray *)array relyonView:(id)view {
    
    CGFloat menuWidth = 90;
    
    NSAssert([view isKindOfClass:[UIView class]]||[view isKindOfClass:[UIBarButtonItem class]], @"relyonView必须是UIView或UIBarButtonItem");
    XMBarMenuView *menu = [[XMBarMenuView alloc] initWithArray:array width:menuWidth relyonView:view];
    return menu;
}
...
- (void)calculateArrowAndFrame{
    ...
    width = _menuWidth;
    height = (_array.count > _maxDisplayCount) ? _maxDisplayCount * _menuCellHeight + kArrowHeight: _array.count * _menuCellHeight + kArrowHeight;
    // 默认在中间
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    XMBarMenuCell *cell = [XMBarMenuCell barMenuCellWithTableView:tableView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(barMenuView:didSelectRowAtIndexPath:)]) {
        [self.delegate barMenuView:self didSelectRowAtIndexPath:indexPath];
    }
}
...
- (void)setMaxDisplaXMount:(NSInteger)maxDisplaXMount{
    if (_maxDisplayCount == maxDisplaXMount)return;
    _maxDisplayCount = maxDisplaXMount;
    _needReload = YES;
}
